...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getFieldListLabel wDataDigger 
FUNCTION getFieldListLabel RETURNS CHARACTER
...
PROCEDURE copyLabelDataDataToClipboard :
  ...
  */
  ...
  IF NUM-ENTRIES(ghDataBrowse:PRIVATE-DATA,CHR(1)) = 0 THEN RETURN.
  cColumnValue = getFieldListLabel(ENTRY(1, ghDataBrowse:PRIVATE-DATA,CHR(1))).
  IF cColumnValue <> '' AND cColumnValue <> ? THEN CLIPBOARD:VALUE = cColumnValue.
  
END PROCEDURE. /* copyLabelDataDataToClipboard */
...
PROCEDURE dropFieldMenu :
  ...
  */
  ...
  DO:
    ...
    hFieldLabel = brFields:QUERY:GET-BUFFER-HANDLE(1):BUFFER-FIELD('cLabel'):HANDLE.
    IF VALID-HANDLE(hFieldName) THEN
    DO:
      /* If CTRL is pressed, do not insert the linked value */
      cField  = hFieldName:BUFFER-VALUE.
      
      IF LOOKUP("ALT", GetKeyList() ) <> 0 THEN
      DO:
        CASE cField:
          WHEN "RECID" THEN cFieldLabel = SUBSTITUTE('RECID(&1)', gcTable).
          WHEN "ROWID" THEN cFieldLabel = SUBSTITUTE('ROWID(&1)', gcTable).
          OTHERWISE cFieldLabel  = hFieldLabel:BUFFER-VALUE.
        END CASE.
        IF cFieldLabel <> '' AND cFieldLabel <> ? THEN CLIPBOARD:VALUE = cFieldLabel.
      END.
      
      ELSE IF LOOKUP("CTRL", GetKeyList() ) <> 0 OR getLinkInfo(cField) = "" THEN
      DO:
        CASE cField:
      ...
      /* No text selected */
      IF cFieldLabel = "" OR cFieldLabel = ? THEN DO:
        IF ficWhere:SELECTION-TEXT = "" THEN
        DO:
...
PROCEDURE getFilterQuery :
  ...
  */
  ...
  FOR EACH bField WHERE bField.lShow = TRUE
    ...
    MESSAGE "BREAK1" VIEW-AS ALERT-BOX.
...
PROCEDURE initVisuals :
  ...
  */
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    brFields     :TOOLTIP = brFields:TOOLTIP + "~n(ALT-RIGHT-CLICK) field label to clipboard".
...
PROCEDURE reopenDataBrowse :
  ...
  */
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    */
    ghDataQuery:QUERY-OPEN() NO-ERROR.
    iStartTime = ETIME.
...
PROCEDURE reopenDataBrowse-create :
  ...
  */
  ...
  DEFINE VARIABLE cFormatNameList as CHARACTER   NO-UNDO.
...
END TRIGGERS.
      ...
      DO:
        ...
        */
        ...
        DO:
          ...
          hField:FORMAT = IF bField.cDataType = "CHARACTER" THEN "x(999)" ELSE cMyFormat  NO-ERROR.
          IF ERROR-STATUS:ERROR THEN
          DO:
        ...
        IF bField.cDataType = "CHARACTER" THEN DO:
          ASSIGN
            cTemp = REPLACE(cMyFormat, "(", "")
            cTemp = REPLACE(cTemp, ")", "")
            cTemp = REPLACE(cTemp, "x", "")
            cFormatList = cFormatList +
            (IF cFormatList <> "" 
            THEN "," 
            ELSE "") + cTemp
            cFormatNameList = cFormatNameList +
            (IF cFormatNameList <> "" 
            THEN "," 
            ELSE "") + bColumn.cFullname
            cTemp = "".
  ...
  */
  ...
  FOR EACH bColumn WHERE VALID-HANDLE(bColumn.hColumn):
    ...
    END CASE.
    IF LOOKUP(bColumn.cFullName, cFormatNameList) > 0 THEN DO:
      iColumnWidth = FONT-TABLE:GET-TEXT-WIDTH-PIXELS(bColumn.hColumn:NAME,getFont("default")) + 75.
    END.
...
PROCEDURE selectClickedRow :
  ...
  */
  ...
  DEFINE VARIABLE v-columnLabel    AS CHARACTER NO-UNDO.
  ...
  DO:
    ...
    DO iColumn = 1 TO phBrowse:NUM-COLUMNS:
      ...
      v-columnLabel = hBrowseColumn:LABEL.
    ...
    phBrowse:PRIVATE-DATA = pcColumnName + CHR(1) + cColumnValue + CHR(1) + v-columnLabel + CHR(1).
...
FUNCTION getFieldList RETURNS CHARACTER
  ...
  */
  ...
  IF pcSortBy = "" OR pcSortBy = ? THEN QUERY qField:QUERY-PREPARE('for each ttField').
  ELSE QUERY qField:QUERY-PREPARE(SUBSTITUTE('for each ttField by &1', pcSortBy)).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getFieldListLabel wDataDigger 
FUNCTION getFieldListLabel RETURNS CHARACTER
  ...
  */
  ...
  QUERY qField:QUERY-PREPARE(SUBSTITUTE('for each ttField where ttField.cFieldName = ' + QUOTER(pcFieldName))).
  QUERY qField:QUERY-OPEN().
  QUERY qField:GET-FIRST().
  ...
  {&timerStop}
END FUNCTION. /* getFieldListLabel */
